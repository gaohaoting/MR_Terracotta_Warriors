...
namespace RayFire
{
    ...
    public class RFMaterialPresets
    {
        ...
        {
            if (materialType == MaterialType.Concrete)
                return concrete.dynamicFriction;
            if (materialType == MaterialType.Brick)
                return brick.dynamicFriction;
            if (materialType == MaterialType.Glass)
                return glass.dynamicFriction;
            if (materialType == MaterialType.Rubber)
                return rubber.dynamicFriction;
            if (materialType == MaterialType.Ice)
                return ice.dynamicFriction;
            if (materialType == MaterialType.Wood)
                return wood.dynamicFriction;
            if (materialType == MaterialType.HeavyMetal)
                return heavyMetal.dynamicFriction;
            if (materialType == MaterialType.LightMetal)
                return lightMetal.dynamicFriction;
            if (materialType == MaterialType.DenseRock)
                return denseRock.dynamicFriction;
            if (materialType == MaterialType.PorousRock)
                return porousRock.dynamicFriction;
            return 0.5f;
        }
        ...
        {
            if (materialType == MaterialType.Concrete)
                return concrete.staticFriction;
            if (materialType == MaterialType.Brick)
                return brick.staticFriction;
            if (materialType == MaterialType.Glass)
                return glass.staticFriction;
            if (materialType == MaterialType.Rubber)
                return rubber.staticFriction;
            if (materialType == MaterialType.Ice)
                return ice.staticFriction;
            if (materialType == MaterialType.Wood)
                return wood.staticFriction;
            if (materialType == MaterialType.HeavyMetal)
                return heavyMetal.staticFriction;
            if (materialType == MaterialType.LightMetal)
                return lightMetal.staticFriction;
            if (materialType == MaterialType.DenseRock)
                return denseRock.staticFriction;
            if (materialType == MaterialType.PorousRock)
                return porousRock.staticFriction;
            return 0.5f;
        }
        ...
        {
            if (materialType == MaterialType.Concrete)
                return concrete.bounciness;
            if (materialType == MaterialType.Brick)
                return brick.bounciness;
            if (materialType == MaterialType.Glass)
                return glass.bounciness;
            if (materialType == MaterialType.Rubber)
                return rubber.bounciness;
            if (materialType == MaterialType.Ice)
                return ice.bounciness;
            if (materialType == MaterialType.Wood)
                return wood.bounciness;
            if (materialType == MaterialType.HeavyMetal)
                return heavyMetal.bounciness;
            if (materialType == MaterialType.LightMetal)
                return lightMetal.bounciness;
            if (materialType == MaterialType.DenseRock)
                return denseRock.bounciness;
            if (materialType == MaterialType.PorousRock)
                return porousRock.bounciness;
            return 0.5f;
        }
