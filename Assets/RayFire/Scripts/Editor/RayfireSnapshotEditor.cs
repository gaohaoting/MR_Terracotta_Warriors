...
namespace RayFire
{
    ...
    public class RayfireSnapshotEditor : Editor
    {
        ...
        void UI_Save()
        {
            ...
            bool compress = EditorGUILayout.Toggle (gui_saveComp, snap.compress);
            if (EditorGUI.EndChangeCheck())
            {
                Undo.RecordObjects (targets, gui_saveComp.text);
                foreach (RayfireSnapshot scr in targets)
                {
                    scr.compress = compress;
                    SetDirty (scr);
                }
            }
        }
        ...
        void UI_Load()
        {
            ...
            EditorGUI.BeginChangeCheck();
            float sizeFilter = EditorGUILayout.Slider (gui_loadSize, snap.sizeFilter, 0f, 1f);
            if (EditorGUI.EndChangeCheck() == true)
            {
                Undo.RecordObjects (targets, gui_loadSize.text);
                foreach (RayfireSnapshot scr in targets)
                {
                    scr.sizeFilter = sizeFilter;
                    SetDirty (scr);
                }
            }
