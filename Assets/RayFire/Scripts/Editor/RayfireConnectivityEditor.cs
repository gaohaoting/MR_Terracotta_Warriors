...
namespace RayFire
{
    ...
    public class RayfireConnectivityEditor : Editor
    {
        ...
        void UI_Preview()
        {
            ...
            bool showGizmo = GUILayout.Toggle (conn.showGizmo, " Show Gizmo ", "Button", GUILayout.Height (25));
            ...
            EditorGUI.BeginChangeCheck();
            bool showConnections = GUILayout.Toggle (conn.showConnections, "Show Connections",    "Button", GUILayout.Height (25));
            if (EditorGUI.EndChangeCheck() == true)
            {
                Undo.RecordObjects (targets, "Show Connections");
                foreach (RayfireConnectivity scr in targets)
                {
                    scr.showConnections = showConnections;
                    SetDirty (scr);
            ...
            EditorGUI.BeginChangeCheck();
            bool showNodes = GUILayout.Toggle (conn.showNodes,       "Show Nodes", "Button", GUILayout.Height (25));
            if (EditorGUI.EndChangeCheck() == true)
            {
                Undo.RecordObjects (targets, "Show Nodes");
                foreach (RayfireConnectivity scr in targets)
                {
                    scr.showNodes = showNodes;
                    SetDirty (scr);
                }
            }
        ...
        void UI_Connectivity () 
        {
            ...
            EditorGUI.BeginChangeCheck();
            ConnectivityType type = (ConnectivityType)EditorGUILayout.EnumPopup (gui_con_type, conn.type);
            if (EditorGUI.EndChangeCheck() == true)
            {
                Undo.RecordObjects (targets, gui_con_type.text);
                foreach (RayfireConnectivity scr in targets)
                {
                    scr.type = type;
                    SetDirty (scr);
            ...
            if (conn.type != ConnectivityType.ByTriangles && conn.type != ConnectivityType.ByPolygons)
            {
                ...
                EditorGUI.BeginChangeCheck();
                float expand = EditorGUILayout.Slider (gui_con_exp, conn.expand, 0, 1f);
                if (EditorGUI.EndChangeCheck() == true)
                {
                    Undo.RecordObjects (targets, gui_con_exp.text);
                    foreach (RayfireConnectivity scr in targets)
                    {
                        scr.expand = expand;
                        SetDirty (scr);
        ...
        void UI_Filters () 
        {
            ...
            {
                EditorGUI.BeginChangeCheck();
                float minimumArea = EditorGUILayout.Slider (gui_filt_area, conn.minimumArea, 0, 1f);
                if (EditorGUI.EndChangeCheck() == true)
                {
                    Undo.RecordObjects (targets, gui_filt_area.text);
                    foreach (RayfireConnectivity scr in targets)
                    {
                        scr.minimumArea = minimumArea;
                        SetDirty (scr);
                    }
                }
            ...
            EditorGUI.BeginChangeCheck();
            float minimumSize = EditorGUILayout.Slider (gui_filt_size, conn.minimumSize, 0, 10f);
            if (EditorGUI.EndChangeCheck() == true)
            {
                Undo.RecordObjects (targets, gui_filt_size.text);
                foreach (RayfireConnectivity scr in targets)
                {
            ...
            EditorGUI.BeginChangeCheck();
            int percentage = EditorGUILayout.IntSlider (gui_filt_perc, conn.percentage, 0, 100);
            if (EditorGUI.EndChangeCheck() == true)
            {
                Undo.RecordObjects (targets, gui_filt_perc.text);
                foreach (RayfireConnectivity scr in targets)
                {
                    scr.percentage = percentage;
                    SetDirty (scr);
                }
            }
            ...
            if (conn.percentage > 0)
            {
                ...
                EditorGUI.BeginChangeCheck();
                int seed = EditorGUILayout.IntSlider (gui_filt_seed, conn.seed, 0, 100);
                if (EditorGUI.EndChangeCheck() == true)
                {
                    Undo.RecordObjects (targets, gui_filt_seed.text);
                    foreach (RayfireConnectivity scr in targets)
                    {
                        scr.seed = seed;
                        SetDirty (scr);
        ...
        void UI_Cluster()
        {
            ...
            EditorGUI.BeginChangeCheck();
            bool clusterize = EditorGUILayout.Toggle (gui_cls_cst, conn.clusterize);
            if (EditorGUI.EndChangeCheck())
            {
                Undo.RecordObjects (targets, gui_cls_cst.text);
                foreach (RayfireConnectivity scr in targets)
                {
                    scr.clusterize = clusterize;
                    SetDirty (scr);
                }
            }
            ...
            if (conn.clusterize == true)
            {
                ...
                EditorGUI.BeginChangeCheck();
                bool demolishable = EditorGUILayout.Toggle (gui_cls_dml, conn.demolishable);
                if (EditorGUI.EndChangeCheck())
                {
                    Undo.RecordObjects (targets, gui_cls_dml.text);
                    foreach (RayfireConnectivity scr in targets)
                    {
                        scr.demolishable = demolishable;
                        SetDirty (scr);
                    }
                }
            }
        ...
        void UI_Collapse()
        {
            ...
            EditorGUI.BeginChangeCheck();
            RayfireConnectivity.RFConnInitType startCollapse = (RayfireConnectivity.RFConnInitType)EditorGUILayout.EnumPopup (gui_clp_init, conn.startCollapse);
            if (EditorGUI.EndChangeCheck() == true)
            {
                Undo.RecordObjects (targets, gui_clp_init.text);
                foreach (RayfireConnectivity scr in targets)
                {
                    scr.startCollapse = startCollapse;
                    SetDirty (scr);
                }
            }
            ...
            if (conn.startCollapse == RayfireConnectivity.RFConnInitType.ByIntegrity)
            {
                ...
                EditorGUI.BeginChangeCheck();
                int collapseByIntegrity = EditorGUILayout.IntSlider (gui_clp_intg, conn.collapseByIntegrity, 1, 99);
                if (EditorGUI.EndChangeCheck() == true)
                {
                    Undo.RecordObjects (targets, gui_clp_intg.text);
                    foreach (RayfireConnectivity scr in targets)
                    {
                        scr.collapseByIntegrity = collapseByIntegrity;
                        SetDirty (scr);
                    }
                }
            }
            ...
            if (exp_collapse == true)
            {
                ...
                EditorGUI.BeginChangeCheck();
                RFCollapse.RFCollapseType type = (RFCollapse.RFCollapseType)EditorGUILayout.EnumPopup (gui_clp_type, conn.collapse.type);
                if (EditorGUI.EndChangeCheck() == true)
                {
                    Undo.RecordObjects (targets, gui_clp_type.text);
                    foreach (RayfireConnectivity scr in targets)
                    {
                        scr.collapse.type = type;
                        SetDirty (scr);
                    }
                }
                ...
                EditorGUI.BeginChangeCheck();
                int start = EditorGUILayout.IntSlider (gui_clp_str, conn.collapse.start, 0, 99);
                if (EditorGUI.EndChangeCheck() == true)
                {
                    Undo.RecordObjects (targets, gui_clp_str.text);
                    foreach (RayfireConnectivity scr in targets)
                    {
                        scr.collapse.start = start;
                        SetDirty (scr);
                    }
                }
                ...
                EditorGUI.BeginChangeCheck();
                int end = EditorGUILayout.IntSlider (gui_clp_end, conn.collapse.end, 1, 100);
                if (EditorGUI.EndChangeCheck() == true)
                {
                    Undo.RecordObjects (targets, gui_clp_end.text);
                    foreach (RayfireConnectivity scr in targets)
                    {
                        scr.collapse.end = end;
                        SetDirty (scr);
                    }
                }
                ...
                EditorGUI.BeginChangeCheck();
                int steps = EditorGUILayout.IntSlider (gui_clp_step, conn.collapse.steps, 1, 100);
                if (EditorGUI.EndChangeCheck() == true)
                {
                    Undo.RecordObjects (targets, gui_clp_step.text);
                    foreach (RayfireConnectivity scr in targets)
                    {
                        scr.collapse.steps = steps;
                        SetDirty (scr);
                    }
                }
                ...
                EditorGUI.BeginChangeCheck();
                float duration = EditorGUILayout.Slider (gui_clp_dur, conn.collapse.duration, 0, 60f);
                if (EditorGUI.EndChangeCheck() == true)
                {
                    Undo.RecordObjects (targets, gui_clp_dur.text);
                    foreach (RayfireConnectivity scr in targets)
                    {
                        scr.collapse.duration = duration;
                        SetDirty (scr);
                    }
                }
                ...
                if (conn.collapse.type != RFCollapse.RFCollapseType.Random)
                {
                    ...
                    EditorGUI.BeginChangeCheck();
                    int var = EditorGUILayout.IntSlider (gui_clp_var, conn.collapse.var, 0, 100);
                    if (EditorGUI.EndChangeCheck() == true)
                    {
                        Undo.RecordObjects (targets, gui_clp_var.text);
                        foreach (RayfireConnectivity scr in targets)
                        {
                            scr.collapse.var = var;
                            SetDirty (scr);
                        }
                    }
                }
                ...
                EditorGUI.BeginChangeCheck();
                int seed = EditorGUILayout.IntSlider (gui_clp_seed, conn.collapse.seed, 0, 99);
                if (EditorGUI.EndChangeCheck() == true)
                {
                    Undo.RecordObjects (targets, gui_clp_seed.text);
                    foreach (RayfireConnectivity scr in targets)
                    {
                        scr.collapse.seed = seed;
                        SetDirty (scr);
                    }
                }
        ...
        void UI_Stress()
        {
            ...
            EditorGUI.BeginChangeCheck();
            bool enable = EditorGUILayout.Toggle (gui_str_enab, conn.stress.enable);
            if (EditorGUI.EndChangeCheck())
            {
                Undo.RecordObjects (targets, gui_str_enab.text);
                foreach (RayfireConnectivity scr in targets)
                {
                    scr.stress.enable = enable;
                    SetDirty (scr);
                }
            }
            ...
            EditorGUI.BeginChangeCheck();
            bool showStress = EditorGUILayout.Toggle (gui_str_prev, conn.showStress);
            if (EditorGUI.EndChangeCheck())
            {
                Undo.RecordObjects (targets, gui_str_prev.text);
                foreach (RayfireConnectivity scr in targets)
                {
                    scr.showStress = showStress;
                    SetDirty (scr);
                }
            }
            ...
            EditorGUI.BeginChangeCheck();
            RayfireConnectivity.RFConnInitType startStress = (RayfireConnectivity.RFConnInitType)EditorGUILayout.EnumPopup (gui_str_init, conn.startStress);
            if (EditorGUI.EndChangeCheck() == true)
            {
                Undo.RecordObjects (targets, gui_str_init.text);
                foreach (RayfireConnectivity scr in targets)
                {
                    scr.startStress = startStress;
                    SetDirty (scr);
                }
            }
            ...
            if (conn.startStress == RayfireConnectivity.RFConnInitType.ByIntegrity)
            {
                ...
                EditorGUI.BeginChangeCheck();
                int stressByIntegrity = EditorGUILayout.IntSlider (gui_clp_intg, conn.stressByIntegrity, 1, 99);
                if (EditorGUI.EndChangeCheck() == true)
                {
                    Undo.RecordObjects (targets, gui_clp_intg.text);
                    foreach (RayfireConnectivity scr in targets)
                    {
                        scr.stressByIntegrity = stressByIntegrity;
                        SetDirty (scr);
                    }
                }
            }
        ...
        void UI_Stress_Properties()
        {
            ...
            EditorGUI.BeginChangeCheck();
            int threshold = EditorGUILayout.IntSlider (gui_str_thr, conn.stress.threshold, 1, 1000);
            if (EditorGUI.EndChangeCheck() == true)
            {
                Undo.RecordObjects (targets, gui_str_thr.text);
                foreach (RayfireConnectivity scr in targets)
                {
                    scr.stress.threshold = threshold;
                    SetDirty (scr);
                }
            }
            ...
            EditorGUI.BeginChangeCheck();
            float erosion = EditorGUILayout.Slider (gui_str_ero, conn.stress.erosion, 0, 10f);
            if (EditorGUI.EndChangeCheck() == true)
            {
                Undo.RecordObjects (targets, gui_str_ero.text);
                foreach (RayfireConnectivity scr in targets)
                {
                    scr.stress.erosion = erosion;
                    SetDirty (scr);
                }
            }
            ...
            EditorGUI.BeginChangeCheck();
            float interval = EditorGUILayout.Slider (gui_str_int, conn.stress.interval, 0.1f, 10f);
            if (EditorGUI.EndChangeCheck() == true)
            {
                Undo.RecordObjects (targets, gui_str_int.text);
                foreach (RayfireConnectivity scr in targets)
                {
                    scr.stress.interval = interval;
                    SetDirty (scr);
                }
            }
            ...
            EditorGUI.BeginChangeCheck();
            int support = EditorGUILayout.IntSlider (gui_str_sup, conn.stress.support, 0, 90);
            if (EditorGUI.EndChangeCheck() == true)
            {
                Undo.RecordObjects (targets, gui_str_sup.text);
                foreach (RayfireConnectivity scr in targets)
                {
                    scr.stress.support = support;
                    SetDirty (scr);
                }
            }
            ...
            EditorGUI.BeginChangeCheck();
            bool exposed = EditorGUILayout.Toggle (gui_str_exp, conn.stress.exposed);
            if (EditorGUI.EndChangeCheck())
            {
                Undo.RecordObjects (targets, gui_str_exp.text);
                foreach (RayfireConnectivity scr in targets)
                {
                    scr.stress.exposed = exposed;
                    SetDirty (scr);
                }
            }
            ...
            EditorGUI.BeginChangeCheck();
            bool bySize = EditorGUILayout.Toggle (gui_str_siz, conn.stress.bySize);
            if (EditorGUI.EndChangeCheck())
            {
                Undo.RecordObjects (targets, gui_str_siz.text);
                foreach (RayfireConnectivity scr in targets)
                {
                    scr.stress.bySize = bySize;
                    SetDirty (scr);
                }
            }
        ...
        void UI_Trigger()
        {
            ...
            if (conn.clusterize == true)
            {
                ...
                EditorGUI.BeginChangeCheck();
                int triggerDebris = EditorGUILayout.IntSlider (gui_trg_deb, conn.triggerDebris, 0, 50);
                if (EditorGUI.EndChangeCheck() == true)
                {
                    Undo.RecordObjects (targets, gui_trg_deb.text);
                    foreach (RayfireConnectivity scr in targets)
                    {
                        scr.triggerDebris = triggerDebris;
                        SetDirty (scr);
                    }
                }
            }
